...
function delRecord(id) {
    $.get('/del/' + id, function() {
        Materialize.toast('Удалено', 1000);
        $("#card" + id).animate({
            opacity: 'toggle',
            height: 'toggle'
        }, {
...
function changeRecord(id) {
    ...
    // Edit html view
    var cardID = "#card" + id;
    $(cardID + " .title-card").text(data.title);
    $(cardID + " .title-card").attr('href', data.url);
    $(cardID + " .about-card").text(data.about);
    $(cardID + " .link").text(data.url.replace(/.*?:\/\//g, ""));
    $(cardID + " .link").attr('href', data.url);
    $(cardID + " .card-image a").attr('href', data.url);
    $(cardID + " .card-image img").attr('src', data.img);

    $(cardID + " .tags .non-style").remove();
    var tag = data.tags.split(',').slice(0, 3);
    for (var x = Math.min(3, tag.length); x--;) {
        var html = '<a href="/tag/' + encodeURI(tag[x]) + '" class="non-style"> <div class="chip z-depth-1">' + tag[x] + '</div> </a>';
        $(cardID + " .tags").prepend(html);
    }
    ...
    $.ajax({
        ...
        dataType: 'json',
        complete: function() {
            Materialize.toast('Сохранено', 500);
        }
    });
}
...
function newRecord() {
    ...
    $.ajax({
        ...
        dataType: 'json',
        complete: function() {
            Materialize.toast('Добавлено', 500);
...
function GetGenerate() {
        ...
        url: "/gen",
        complete: function() {
            Materialize.toast('Сгенерировано', 500);
        }
    });
}